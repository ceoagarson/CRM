...
export default function IMachineWiseReportPage({ startDate, endDate, machinesData }: Props) {
  ...
  const MemoColumns: Column<IMachineWiseReport>[] = React.useMemo(
    () => [
      ...
      //Machine20
      {
        Header: "PU MACHINE-21",
        accessor: 'm21',
        Cell: (props) => {
          return (
            <Typography sx={{ textTransform: "capitalize" }}>{props.row.original.machines[20]?props.row.original.machines[20].production:"0"}</Typography>
          )
        },
        Footer: (props) => {
          let totalm20 = 0
          props.rows.map((row) => {
            totalm20 += Number(row.original.machines[20]?row.original.machines[20].production:"0")
            return null
